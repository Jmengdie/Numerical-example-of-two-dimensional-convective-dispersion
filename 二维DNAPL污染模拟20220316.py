...
def LJY_alc_con(x, z):#x,z分别表示场地的水平距离和深度(m)
    ...
    if yorn=='y':
        ...
        n = float(input('请输入土壤孔隙率\n'))
        aL = float(input('请输入纵向弥散度（以 m 计）\n'))
        aT = float(input('请输入垂向弥散度（以 m 计）\n'))
        Rd = float(input('请输入阻滞因子\n'))
        ps = float(input('请输入土壤干密度，以 kg/m3 计)'))
        Kd = float(input('请输入分配系数，以 m3/kg 计'))
    ...
    for k in range(N):
        ...
        for i in range(1,X-1):
            ...
            for j in range(1,Z-1):
                i=int(i);
                j=int(j);
                yk[i,j]=float((1+dt*kk)*y[i,j] + dt*Dz*Rd*(y[i,j+1]-2*y[i,j]+y[i,j-1])/dxz/dxz + dt*Dx*Rd*(y[i+1,j]-2*y[i,j]+y[i-1,j])/dxz/dxz-dt*u*(y[i+1,j]-y[i-1,j])/2/n/Rd/dxz-dt*w*(y[i,j+1]-y[i,j-1])/2/n/Rd/dxz);
